...
To resolve instructions from session context (auth, tenant, channel, or external data), turn context, or step-local state, wrap `defineInstructions` in `defineDynamic`. System-role instruction resolvers support `session.started`, `turn.started`, and `step.started`, and may return `null` to contribute nothing. User-role results support session and turn scope only.
...
Step-scoped system instructions resolve before each model call and replace the same file's wider-scope value for that call. See [Dynamic capabilities](./guides/dynamic-capabilities) for examples and failure behavior.
