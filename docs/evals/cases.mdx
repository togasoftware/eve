...
- `t.cancel()` requests cooperative cancellation of the primary session's active turn. Both `accepted` and `no_active_turn` are successful outcomes; the latter means no cancellation hook admitted the request and can be transient during turn startup.
