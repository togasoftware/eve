...
Once `send()` is accepted, the session exposes its assigned `sessionId` and can request cooperative cancellation with `session.cancel()`, even while the response stream is still running. The result status is `accepted` when the active turn admitted the signal or `no_active_turn` when no cancellation hook admitted it. The latter can be transient during turn startup or mean the turn already settled:
