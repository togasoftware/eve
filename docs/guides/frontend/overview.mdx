...
export function Chat() {
  ...
  return (
    ...
    >
      ...
      <button disabled={!isBusy} onClick={agent.stop} type="button">
        Stop
      </button>
    </form>
  );
...
| `stop`    | Request best-effort cancellation while keeping its event stream attached.      |
...
Assistant text, reasoning, tool calls, and tool results stream into `data` as they arrive, and `status` moves from `ready` to `submitted` to `streaming` and back. Call `stop()` to request cancellation of the active turn started by this binding. The hook retries transient admission failures and keeps reading until the turn settles as `turn.cancelled` followed by `session.waiting`, so the same conversation can continue safely. An early stop is queued until eve identifies the turn, and a late stop cannot cancel a newer turn. Cancellation is best-effort: a turn that cannot admit cancellation continues normally. Restoring a session does not attach the binding to a turn that was already running.

Unmounting the binding aborts only its local request or stream; durable server work continues. `reset()` also detaches locally, clears local state, and makes the next send start a fresh durable session.
