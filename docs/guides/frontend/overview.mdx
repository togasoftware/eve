...
| `stop`    | Durably cancel the active turn while keeping its event stream attached.        |
...
Assistant text, reasoning, tool calls, and tool results stream into `data` as they arrive, and `status` moves from `ready` to `submitted` to `streaming` and back. Call `stop()` to durably cancel the active turn. The hook keeps reading until the turn settles as `turn.cancelled` followed by `session.waiting`, so the same conversation can continue safely. An early stop is queued until eve identifies the turn, and a late stop cannot cancel a newer turn.
