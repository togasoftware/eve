...
| `stop`    | `() => void`                                | Durably cancel the active turn.                                           |
...
`stop()` durably cancels the active turn and keeps its stream attached through the cancellation boundary. When the binding is disposed, it aborts only its local request or stream while allowing durable server work to continue. `reset()` also detaches locally, wipes local state, and starts a fresh session. To resume across reloads, hand `initialSession` the state you saved earlier and use `onSessionChange` to persist the cursor as it advances:
