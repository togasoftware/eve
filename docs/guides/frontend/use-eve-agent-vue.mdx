...
`stop()` durably cancels the active turn and keeps its stream attached through the cancellation boundary. When the component scope is disposed, it aborts only its local request or stream while allowing durable server work to continue. `reset()` also detaches locally, wipes local state, and starts a fresh session. To survive a reload, pass `initialSession` to restore a saved session and `onSessionChange` to persist the cursor as it moves:
