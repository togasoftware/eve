...
| `stop`    | `() => void`                                 | Request best-effort cancellation of the active turn.                      |
...
`stop()` requests best-effort cancellation of the active turn and keeps its stream attached through the cancellation boundary. A turn that cannot admit cancellation continues normally. When the component scope is disposed, it detaches only its local request or stream while allowing durable server work to continue. `reset()` also detaches locally, wipes local state, and starts a fresh session. Restoring a session does not attach the binding to a turn that was already running. To survive a reload, pass `initialSession` to restore a saved session and `onSessionChange` to persist the cursor as it moves:
