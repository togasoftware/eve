...
Both result statuses are successful. `"accepted"` means a cancellation hook accepted the request, not that the observed turn will cancel; confirmation is `turn.cancelled` followed by `session.waiting` on the event stream. `"no_active_turn"` means no cancellation hook admitted the request. This can be transient during turn startup or permanent for an unknown token, a parked session, or an already-completed turn.
