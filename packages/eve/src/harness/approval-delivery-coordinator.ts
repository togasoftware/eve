...
export async function coordinateApprovalDelivery(input: {
  ...
  for (const response of input.stepInput?.inputResponses ?? []) {
  ...
  const stepInput = removeConsumedResponses(input.stepInput, consumed);
  if (consumed.size > 0) {
    return deliveryResult(session, stepInput, didCommit ? "continue-coordination" : "continue");
  }
  ...
  return didCommit
    ? deliveryResult(session, stepInput, "continue-coordination")
    : deliveryResult(
        session,
        stepInput,
