...
import type { ApprovalResponseAuthorizer } from "#public/definitions/approval.js";
...
describe("coordinateApprovalDelivery", () => {
  ...
  function approvalTool(
    name: string,
    authorizeResponse: ApprovalResponseAuthorizer,
  ): HarnessToolDefinition {
    return {
      approval: { authorizeResponse, policy: () => "user-approval" },
      description: name,
      inputSchema: jsonSchema({ type: "object" }),
      name,
    };
  ...
  it("durably settles an allowed candidate before ordinary approval resolution", async () => {
    const tools: HarnessToolMap = new Map([
      ["create_issue", approvalTool("create_issue", () => "allowed")],
    ]);
    const result = await approvalContext(() =>
      completeApprovalDelivery({
        now: 100,
  ...
  it("processes every approval response in a delivery", async () => {
    ...
    const tools: HarnessToolMap = new Map([
      [
        "create_issue",
        {
          ...approvalTool("create_issue", () => "allowed"),
        },
      ],
      [
        "delete_issue",
        {
          approval: {
            authorizeResponse: () => ({ safeReason: "Delete denied.", status: "rejected" }),
  ...
  it("assigns a fresh attempt id when the same responder retries", async () => {
    const tools: HarnessToolMap = new Map([
      [
        ...
        {
          approval: {
            authorizeResponse: () => ({ safeReason: "Retry allowed.", status: "rejected" }),
            policy: () => "user-approval",
          },
          description: "Create issue",
          inputSchema: jsonSchema({ type: "object" }),
